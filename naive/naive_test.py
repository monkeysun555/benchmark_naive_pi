...
def update_naive_rec(pred_tp, new_tp):
...
def t_main():
	...
	for i in range(len(cooked_times)):
		...
		naive_tp_rec = [INIT_BW] * NAIVE_PRE_STEP
		...
		for i in range(TEST_DURATION):
			...
			while True:  # serve video forever
				...
				take_action = 0
				current_naive_tp += chunk_number/CHUNK_IN_SEG * real_chunk_size / download_duration
				...
				if next_chunk_idx == 0 or sync:
					take_action = 1
					naive_tp_rec = update_naive_rec(naive_tp_rec, current_naive_tp * KB_IN_MB)
