...
MS_IN_S = 1000.0
INIT_BUFFER = 1000.0
RESULT_DIR = './test_results/naive/' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's/'
FIGURES_DIR = './test_figures/naive/' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's/'
RESULT_FILE = './test_figures/naive/'+ str(int(SERVER_START_UP_TH/MS_IN_S)) + 's/'
...
def main():
	...
	n_start_init_time = ['start_init_time']
	...
	for i in range(len(file_records)):
		...
		#For buffer
		time_trace = [float(info[0]) for info in records]
		...
		n_start_init_time.append(time_trace[1] - ReLU(INIT_BUFFER-buffer_trace[1]))
	...
	if SAVE:
		...
		for i in range(len(n_files)):
			...
			current_log.append(n_start_init_time[i])
