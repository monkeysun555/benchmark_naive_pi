...
if not IF_ALL_TESTING:
	...
	LOG_FILE = LOG_FILE_DIR + '/PI_' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's'
else:
	if IF_NEW:
		LOG_FILE_DIR = './all_test_results'
		LOG_FILE = LOG_FILE_DIR + '/PI_' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's'
		ALL_TESTING_DIR = '../../algorithms/all_results/'
		ALL_TESTING_FILE = ALL_TESTING_DIR + 'PI_' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's.txt'
	else:
...
def estimate_bw(bw_history):
	weight_sum = np.sum([(i+1)*bw_history[i] for i in range(len(bw_history))])
	return weight_sum/np.sum(range(len(bw_history)+1)) # in mbps or kbpms

def estimate_bw_h(bw_history):
	return len(bw_history)/np.sum([1/tp for tp in bw_history])

def update_pi_rec(pred_tp, new_tp):
	...
	for i in range(len(cooked_times)):
		...
		for i in range(TEST_DURATION):
			...
			while True:  # serve video forever
				...
				take_action = 0
				current_pi_tp += chunk_number/CHUNK_IN_SEG * real_chunk_size / download_duration
				...
				if next_chunk_idx == 0 or sync:
					...
					take_action = 1
					pi_tp_rec = update_pi_rec(pi_tp_rec, current_pi_tp * KB_IN_MB)
